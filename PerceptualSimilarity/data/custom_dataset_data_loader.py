...
from PerceptualSimilarity.data.base_data_loader import BaseDataLoader
import os

def CreateDataset(dataroots,dataset_mode='2afc',load_size=64,):
    dataset = None
    if dataset_mode=='2afc': # human judgements
        from PerceptualSimilarity.data.dataset.twoafc_dataset import TwoAFCDataset
        dataset = TwoAFCDataset()
    elif dataset_mode=='jnd': # human judgements
        from PerceptualSimilarity.data.dataset.jnd_dataset import JNDDataset
        dataset = JNDDataset()
    else:
...
class CustomDatasetDataLoader(BaseDataLoader):
    ...
    def initialize(self, datafolders, dataroot='../Training/bokeh',dataset_mode='2afc',load_size=64,batch_size=1,serial_batches=True, nThreads=1):
